...
/* 5/27/22: cae puts in a 'scan' command. */ 
static void action_cat(char *infile) {
  ...
  /* idea: change this to switch/case UNIXCRYPT, DECRYPT, KEYSCAN */
  switch (cmd.mode) {
  case UNIXCRYPT:
    r = unixcrypt_streams(fin, stdout, cmd.keyword);
    break;
  case SCAN:
    r = cckeyscan_streams(fin, stdout, cmd.keyword);
    if (r) fprintf(stderr, "keyscan position: %d\n",r);
    /* to do: return r somehow?? */
    /*  oh.  we do not want to use 'r' any more. let it be 0 */
    break;
  default:
...
/* it is not  (cmd.mode==ENCRYPT || cmd.mode==DECRYPT || cmd.mode==KEYCHANGE)*/
    if (cmd.mode==SCAN) {
      if (1==0) {
	fprintf(stderr,"\naction_cat for scan:\n");
      }
