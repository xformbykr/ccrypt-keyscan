...
/* called by streamhandler, who fills the buffer */
int cckeyscan(ccrypt_stream_t *b) {
  ...
  if (1==0) {
    fprintf(stderr, "call cckeyscan() \n");
  }

  int stride = 32; /* 128;*/
  /* spin up to the first stride */
  /*  while ( b->avail_in > STRIDE &&
      ((b->stream_position % STRIDE) || b->stream_position == 0)) */
  /* while ( b->stream_position % stride || b->stream_position == 0) */
  while ( b->stream_position % stride ) 
    {
      b->stream_position++;
      b->next_in++;
      b->avail_in--;
    }
  /* when we get here, we are in the stream on a multiple of STRIDE */
  if (1==0) {
    fprintf(stderr,"b values after spin: %ld, %d\n",b->stream_position,
	    b->avail_in);
  }
  ...
  while ( b->avail_in > 32 ) {
    ...
    for (i=0; i<st->n; i++) {
      /* check the "magic number" */
      memcpy(lbuf, (char *)(b->next_in), 32);
      xrijndaelDecrypt(lbuf, &st->rkks[i]);
      if ( strncmp((char *)lbuf, MAGIC, 4) == 0) {
	/* key matches */
	fprintf(stderr,	"  found magic at stream position %ld\n",
		b->stream_position);
	break;  /* out of for loop */
      } /* end if strncmp */
    }   /* end for loop  */      
    
    b->stream_position += stride;
    b->next_in += stride;
    b->avail_in -= stride;
    
  }  /* end while(b->avail_in>32) */

  /* finish up any avail_in bytes in order for continuing stream processing */
  if (b->avail_in > 0) {
    b->stream_position += b->avail_in;
    b->avail_in = 0;
  }
  /*
  b->next_in += b->avail_in;  NO, this will be overwritten
  b->avail_in = 0;  this must be zero on return, for next read. 
  */
  return 0;
}
