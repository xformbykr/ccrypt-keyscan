...
static int streamhandler(ccrypt_stream_t *b, workfun *work, endfun *end, 
  ...
  /*  if cmd.seek_to then perform lseek here but change 'scan' to nbytes */
  /*  NB, gotta be sure we can even do this in stream! */

  
  if (cmd.seekbytes) {
    r = fseek( fin, cmd.seekbytes, 0);
    if (r || ferror(fin)) {
  ...
  /*  keep running totalof bytes; totalbytes = 0 */
  totalbytes = 0; 
  if (cmd.totalbytes) {
    maxbytes = cmd.totalbytes;
  }
  else {
    maxbytes = 2e12;     /*268435459;*/
  }
  
  while (1) {
  /*  NB change 'scan' to nbytes */
    ...
    r = work(b);
    if (1==0) {
      fprintf(stderr,"work(b) returned %d %ld\n",r,totalbytes);
    }
    if (r) {
      fprintf(stderr, "scan work() returned %d\n",r);
      /* we may want to 'getfilepos' or something, report it. NOPE. */ 
      return r;
    }
    /** put missing code back here **/
...
int ccdecrypt_streams(FILE *fin, FILE *fout, const char *key) {
  ...
  fprintf(stderr, "call ccdecrypt_streams\n");
...
int cckeyscan_streams(FILE *fin, FILE *fout, const char *key) {
  ...
  fprintf(stderr, "  call cckeyscan_streams\n");
  ...
  b->stream_position = 0;  /*  for now; please fix arithmetic */
  /*                      cckeyscan               */
  /*  to do:  try phrasing these as with the others, hoping for
      continued processing.???? */

  if ( 1 == 1 ) {
    return streamhandler(b, cckeyscan, cckeyscan_end, fin, fout);
  } else {
    rv = streamhandler(b, cckeyscan, cckeyscan_end, fin, fout);
    fprintf(stderr, "final b->stream_position: %ld\n", b->stream_position);
    return rv;
  }
}
...
int ccdecrypt_file(int fd, const char *key) {
  ...
  if ( 1 == 0 ) {
    fprintf(stderr, "call ccdecrypt_file\n");
  }
