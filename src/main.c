...
"    -n  --scan            scan file for iv matching key\n"
"\n"
"Options:\n"
...
"    -n, --scan            scan file for Iv from key\n "
"   -B, --fseek nBytes    number Bytes to fseek into file if scanning\n "
...
static void output_commandline(cmdline cmd, FILE *fout) {
  ...
  const char *mode[] = {"encrypt", "decrypt", "keychange", "cat", "unixcrypt", "scan"};
  ...
  fprintf(fout, "scan = %s\n", cmd.scan ? "yes" : "no" );
  fprintf(fout, "fseek bytes = %d\n", cmd.seekbytes ? 0 : cmd.seekbytes );
  fprintf(fout, "filter = %s\n", cmd.filter ? "yes" : "no");
...
static const char *shortopts = "edcxuhVLvqDfmE:K:k:F:H:S:sP:Q:tby:rRlTnB-";
  ...
  while ((c = getopt_long(ac, av, shortopts, longopts, NULL)) != -1) {
	...
	*p = 0;
    ...
    case 'n':
      cmd.scan = 1;
      cmd.mode = SCAN;
      if (1==1) fprintf(stderr,"We will scan for IV per key.\n");
      break;
    case 'B': 
      cmd.seekbytes = 2000000000; /* atoi(optarg); */
      cmd.mode = SCAN;
      fprintf(stderr,"wish to seek %d\n",cmd.seekbytes);
      break;
    case '?':
